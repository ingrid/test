define(["../jam"], function (_jam) {
	...
	_jam2.default.LevelMap = function (tilesize, w, h, image, indices) {
			...
			for (var y = Math.max(0, y1); y < Math.min(y2 + 1, self.height); ++y) {
				for (var x = Math.max(0, x1); x < Math.min(x2 + 1, self.width); ++x) {
					...
					if (t !== null && self.image) {
						...
						if (dx + self.tilesize > camera.size.x) {
							trimx = dx + self.tilesize - camera.size.x;
						}
						if (dy + self.tilesize > camera.size.y) {
							trimy = dy + self.tilesize - camera.size.y;
						}

						// ugh.
						context.drawImage(self.image, t.imageIndex * self.tilesize - cutx, cuty, self.tilesize + cutx - trimx, self.tilesize + cuty - trimy, dx - cutx, dy - cuty, self.tilesize + cutx - trimx, self.tilesize + cuty - trimy);
					}
				}
			}
		};
